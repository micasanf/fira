...
async function sendEmailWithResend(
  ...
  const { data, error } = await getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL || "FIRA <noreply@fira.com.ph>",
    to: [to],
    subject: subject,
    html: html,
  });
...
async function sendWelcomeEmail(
    ...
    <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="color: #2563eb; margin-bottom: 10px;">🎉 Welcome to FIRA!</h1>
        ...
        <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); padding: 25px; border-radius: 12px; margin: 20px 0;">
          ...
          <p style="margin: 0 0 15px 0;">Welcome to FIRA! We're excited to help you discover amazing career opportunities that match your skills and aspirations.</p>
        </div>
        ...
        <div style="text-align: center; color: #9ca3af; font-size: 14px;">
          ...
          <p style="margin: 0;"><strong>The FIRA Team</strong></p>
        </div>
      </body>
    </html>
  `;
  ...
  return sendEmailWithResend(
    email,
    `🎉 Welcome to FIRA, ${firstName}! Your career journey begins now`,
    welcomeContent
  );
}
