...
// Safe date parser for any date shape returned by Supabase.
...
// Normalize legacy / supabase statuses into our funnel buckets.
function statusBucket(s: string | undefined): "applied" | "screening" | "interview" | "offer" | "hired" | "rejected" {
  const v = (s || "").toLowerCase();
  if (["approved", "hired"].includes(v)) return "hired";
  if (["offered", "offer"].includes(v)) return "offer";
  if (["interview", "interviewed"].includes(v)) return "interview";
  if (["reviewing", "screening", "shortlisted"].includes(v)) return "screening";
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const { data: opps, error: oppsError } = await supabase
        .from("opportunities")
        .select("id, title, status, posted_at, application_count")
      ...
      // Fetch applications for these opportunities (real counts + funnel/trend data)
      let applicationsData: Application[] = [];
      if (opportunityIds.length > 0) {
        const { data: apps, error: appsError } = await supabase
          .from("applications")
          .select("id, opportunity_id, status, applied_at, submitted_at, created_at")
          .in("opportunity_id", opportunityIds);
        if (appsError) throw appsError;
        applicationsData = (apps || []).map((a: any) => ({
          id: a.id,
      ...
      for (const app of applicationsData) {
        const oid = app.opportunityId;
        if (oid) applicantCountMap[oid] = (applicantCountMap[oid] || 0) + 1;
      }
      // Apply real counts to postings
      for (const p of postingsData) {
        p.applicants = applicantCountMap[p.id] ?? p.applicants ?? 0;
      }

      const totalApplicants = postingsData.reduce((acc, p) => acc + (p.applicants || 0), 0);
      const activeJobs = postingsData.filter((p) => (p.status || "").toLowerCase() === "active").length;
      const avgApplicants =
        totalPostings > 0 ? parseFloat((totalApplicants / totalPostings).toFixed(1)) : 0;
      setStats({ totalPostings, totalApplicants, activeJobs, avgApplicants });

      // --- Bar Chart Data ---
      const sortedPostings = [...postingsData].sort((a, b) => {
        const da = toDate(a.postedAt)?.getTime() || 0;
        const db = toDate(b.postedAt)?.getTime() || 0;
        return db - da;
      });
      ...
      applicationsData.forEach((app) => {
        const d = toDate(app.appliedAt || app.submittedAt || app.createdAt);
        if (!d) return;
      ...
      applicationsData.forEach((app) => {
        ...
        if (bucket === "applied") statusCounts.Applied++;
        else if (bucket === "screening") { statusCounts.Applied++; statusCounts.Screening++; }
        else if (bucket === "interview") { statusCounts.Applied++; statusCounts.Screening++; statusCounts.Interview++; }
        else if (bucket === "offer") { statusCounts.Applied++; statusCounts.Screening++; statusCounts.Interview++; statusCounts.Offer++; }
        else if (bucket === "hired") { statusCounts.Applied++; statusCounts.Screening++; statusCounts.Interview++; statusCounts.Offer++; statusCounts.Hired++; }
      ...
      // --- Time to Hire ---
      const hiredApps = applicationsData.filter((a) => statusBucket(a.status) === "hired");
      if (hiredApps.length > 0) {
        const totalDays = hiredApps.reduce((sum, app) => {
          const submitted = toDate(app.appliedAt || app.submittedAt || app.createdAt) || new Date();
      ...
      const topPostingsData: TopPosting[] = postingsData
        .map((p) => {
          ...
          const interviews = apps.filter((a) => {
            ...
            return b === "interview" || b === "offer" || b === "hired";
          }).length;
          const hired = apps.filter((a) => statusBucket(a.status) === "hired").length;
          return {
            title: p.title,
