...
// public_profiles.benefits is TEXT (scalar). Coerce into a string array for rendering.
function benefitsToArray(benefits: string | string[] | undefined | null): string[] {
  if (!benefits) return [];
  if (Array.isArray(benefits)) {
    return benefits.filter((b) => typeof b === "string" && b.trim().length > 0);
  }
  return benefits
    .split(/[\n,]/)
    .map((s) => s.trim())
...
export default function CompanyPage() {
  ...
  useEffect(() => {
    ...
    const fetchCompanyData = async () => {
      try {
        // Fetch company profile from public_profiles
        ...
        if (profileData) {
          setProfile({ id: profileData.id, ...profileData } as CompanyProfile);
        }
        ...
        if (oppsData) {
          setOpportunities(oppsData as CompanyOpportunity[]);
        }
      } catch (err) {
  ...
  const companyName =
    profile.company_name || profile.display_name || "Company";
  const companyLogo = profile.company_logo_url || profile.photo_url;
  const activePositions = opportunities.filter(
    (o) => o.status !== "closed" && o.status !== "Closed"
  );
  const benefitsList = benefitsToArray(profile.benefits);
                  ...
                  {activePositions.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {activePositions.map((opp) => {
                        const skillList: string[] = Array.isArray(opp.skills)
                          ? opp.skills
                          : [];
                        const postedRaw = opp.posted_at || opp.created_at;
                        const postedDate = postedRaw ? new Date(postedRaw) : new Date();
                        const typeLabel = opp.type || opp.job_type || "Full-time";
